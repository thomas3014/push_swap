...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/04/14 13:08:11 by tlefebvr          #+#    #+#             */
/*   Updated: 2022/04/20 14:22:30 by tlefebvr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_stack *stack_a, t_stack *stack_b)
{
	...
	while (--i > 0 && (stack_a->size - i) < minimal_mvt)
	{
		...
		if ((t[0] > stack_b->size / 2 && t[1] >= (stack_a->size - i)
				&& t[1] < m) || (t[0] > stack_b->size / 2
				&& t[1] < (stack_a->size - i)
				&& (stack_a->size - i) < m)
			|| ((stack_a->size - i) + t[0] < m))
			*min_mvt = t[0];
		less_mvt_at_end_suite(minimal_mvt, stack_a, stack_b);
	}
	return (m);
}
